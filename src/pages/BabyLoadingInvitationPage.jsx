...
import {invitationConfig as config,pregnancyProgress} from '../data/invitationConfig';
...
export default function BabyLoadingInvitationPage(){
  ...
  const loadingPercentage=pregnancyProgress(config.estimatedDueDate);
  ...
  return <main className={`invitation-page loading-page ${view==='card'?'card-preview':''}`}>
    ...
    <section className="loading-card">
      ...
      <div className="loading-copy"><p className="eyebrow">UM NOVO CAPÍTULO ESTÁ A COMEÇAR</p><h1>{config.title}</h1><div className="loading-progress" aria-label={`Baby loading: ${loadingPercentage}%`}><span style={{'--loading-progress':`${loadingPercentage}%`}}/><b>{loadingPercentage}%</b></div>
