...
export default function HomePage(){
  ...
  return <><Header/><main>
    ...
    <section className="section event" id="evento"><div className="section-head"><p className="eyebrow">GUARDA ESTE DIA</p><h2>Todos os detalhes,<br/><em>num só lugar</em></h2></div><div className="details-grid"><article><span>01</span><small>DATA</small><h3>{dateLabel}</h3><p>Uma sexta-feira para celebrarmos juntos.</p></article><article><span>02</span><small>HORA</small><h3>{c.eventTime} — {c.eventEndTime}</h3><p>Chega com tempo para aproveitarmos toda a tarde.</p></article><article><span>03</span><small>LOCAL</small><h3>{c.venueName}</h3><p>{c.address}</p><a href={c.mapsUrl} target="_blank" rel="noreferrer">Abrir no Google Maps ↗</a></article></div><aside className="dress-code" aria-labelledby="dress-code-title"><div className="dress-code-copy"><small>DRESS CODE</small><h3 id="dress-code-title">Tons neutros &amp; terrosos</h3><p>Para tornar este dia ainda mais especial, inspira-te nesta paleta de tons suaves e quentes.</p></div><div className="dress-code-palette" aria-label="Paleta sugerida: castanho, taupe, dourado suave, areia e creme"><i className="dress-brown" title="Castanho"/><i className="dress-taupe" title="Taupe"/><i className="dress-gold" title="Dourado suave"/><i className="dress-sand" title="Areia"/><i className="dress-cream" title="Creme"/></div></aside><Countdown date={c.eventDate} time={c.eventTime}/></section>
    ...
    <section className="section gifts" id="presentes"><div className="section-head centered"><p className="eyebrow">UM MIMINHO PARA O BEBÉ</p><h2>Lista de <em>presentes</em></h2><div className="gift-intro"><strong>A tua presença é, verdadeiramente, o presente mais importante para nós.</strong><p>Não sintas qualquer obrigação de oferecer algo. Mas caso pretendas fazê-lo, preparámos algumas sugestões para ajudar. O link de cada artigo é apenas uma referência — podes adquiri-lo na loja indicada ou noutro local da tua preferência.</p></div></div><GiftRegistry/></section>
