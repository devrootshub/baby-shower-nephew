...
function ConfirmationsPage({data,reload}){
  ...
  const cleanName=name=>(name||'').trim().toLocaleLowerCase('pt-PT');
  const responseForGuest=guest=>data.rsvps.find(r=>r.guestListId===guest.id)||data.rsvps.find(r=>cleanName(r.fullName)===cleanName(guest.name));
  const openDetails=(guest,rsvp)=>{setSelectedGuest(guest);setSelectedRsvp(rsvp);setModal('details')};
  return <>
    <section className="confirmations-panel">
      ...
      <div className="guest-table"><div className="guest-table-head"><span>Convite</span><span>Tipo</span><span>Estado</span><span>Ações</span></div>{filtered.map(g=>{const rsvp=responseForGuest(g);return <article className="guest-table-row" key={g.id}><strong><FormattedName name={g.name}/></strong><span>{g.type==='group'?'Casal / Família':g.type==='child'?'Criança':'Adulto'}</span><StatusPill status={g.status}/><div className="guest-row-actions">{rsvp&&<button className={rsvp.message?'has-message':''} onClick={()=>openDetails(g,rsvp)} aria-label={`Ver detalhes de ${g.name}`}>Detalhes</button>}<button onClick={()=>openEdit(g)} aria-label={`Editar ${g.name}`}>Editar</button><button className="remove" onClick={()=>{setSelectedGuest(g);setModal('delete')}} aria-label={`Eliminar ${g.name}`}>Eliminar</button></div></article>})}</div>
    </section>
    ...
    {modal==='details'&&selectedGuest&&selectedRsvp&&<AdminModal title={`Detalhes de ${selectedGuest.name}`} close={()=>setModal(null)}><div className="rsvp-detail-grid"><div><small>ADULTOS</small><strong>{selectedRsvp.adults}</strong></div><div><small>CRIANÇAS</small><strong>{selectedRsvp.children}</strong></div><div><small>DATA DA RESPOSTA</small><strong>{new Intl.DateTimeFormat('pt-PT',{dateStyle:'medium',timeStyle:'short'}).format(new Date(selectedRsvp.createdAt))}</strong></div><div><small>ORIGEM</small><strong>{selectedRsvp.source==='manual'?'Registo manual':'Website'}</strong></div></div><div className={`rsvp-message ${selectedRsvp.message?'':'empty'}`}><small>MENSAGEM</small><p>{selectedRsvp.message||'Este convidado não deixou nenhuma mensagem.'}</p></div><div className="modal-actions"><button type="button" className="btn primary" onClick={()=>setModal(null)}>Fechar</button></div></AdminModal>}
